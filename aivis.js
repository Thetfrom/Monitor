...
if(v==='yes'){yes++;ans++;}else if(v==='no'){ans++;}}});return {yes:yes,ans:ans,p:ans?pct(yes,ans):null};}
var S=score(rows);
...
+'<span class="pill"><i></i>'+E(v.pill)+'</span></div>'
+'<div class="bd">'
...
+'</svg><div class="dc"><b>'+(S.p===null?'\u2014':(S.p+'%'))+'</b><s>Score</s></div></div>'
+'<div><p class="tr'+trc+'">'+E(tr)+'</p>'
+'<p class="lead">'+E(lead)+'</p>'
+(S.ans?('<p class="body">You were named in '+S.yes+' of '+S.ans+' answered checks today.</p>'):'')
...
var mo=null,iv=null;
try{mo=new MutationObserver(tryMount);mo.observe(document.documentElement,{childList:true,subtree:true});}catch(e){mo=null;}
iv=setInterval(function(){tryMount();if(mo&&iv&&document.getElementById('aivis2')){clearInterval(iv);iv=null;}},600);
