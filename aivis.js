...
function ptxt(r,n){return r['answer_text_kw'+n]||r['answerTextKw'+n]||r['answer_text_'+n]||'';}function kwlab(v,n){if(v&&v.kws&&v.kws[n-1])return String(v.kws[n-1]);var rs=(v.all||[]);for(var i=0;i<rs.length;i++){var k=rs[i]['target_keyword_'+n];if(k)return String(k);}return '';}function hlq(t,w){if(!w)return E(t);var s=String(w),i=t.toLowerCase().indexOf(s.toLowerCase());if(i<0)return E(t);return E(t.slice(0,i))+'<b class="qn">'+E(t.slice(i,i+s.length))+'</b>'+E(t.slice(i+s.length));}function css6(){return '<style>#aivis2 .q6{margin:26px 0 0}#aivis2 .qh{font-size:20px;font-weight:700;letter-spacing:-.06em;margin:0 0 12px;color:var(--tx)}#aivis2 .qc{background:var(--cd);border:1px solid var(--bd);border-radius:14px;padding:20px 22px}#aivis2 .qg+.qg{border-top:1px solid var(--dv);margin-top:18px;padding-top:18px}#aivis2 .qq{color:var(--or);font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;margin:0 0 12px}#aivis2 .qr{display:grid;grid-template-columns:92px 1fr;gap:14px;align-items:start;margin:0 0 14px}#aivis2 .qm2{color:var(--mu);font-size:11px;font-weight:700;letter-spacing:.06em;text-transform:uppercase;padding-top:3px}#aivis2 .qt{font-style:italic;font-size:13.5px;line-height:1.6;color:var(--bo);margin:0}#aivis2 .qn{color:var(--or);font-weight:700;font-style:italic}#aivis2 .qtag{display:inline-block;margin-left:7px;font-style:normal;font-size:10px;font-weight:700;letter-spacing:.04em;text-transform:uppercase;color:var(--mu);border:1px solid var(--dv);border-radius:999px;padding:1px 7px}#aivis2 .qe{color:var(--mu);font-size:13px;margin:0}#aivis2 .qfoot{color:var(--mu);font-size:11.5px;line-height:1.55;margin:14px 0 0;padding-top:12px;border-top:1px solid var(--dv)}@media(max-width:640px){#aivis2 .qr{grid-template-columns:1fr;gap:3px}}</style>';}function trows(v){var a=(v.all||[]),o=[],s={},i,r,m;for(i=0;i<a.length;i++){r=a[i];if(String(r.check_date)!==String(v.today))continue;m=String(r.model||'').toLowerCase();if(!m||s[m])continue;s[m]=1;o.push(r);}return o;}function quotes(v){var rs=trows(v);if(!rs.length)return '';var g=[],i,j,n,any=false;for(n=1;n<=3;n++){var kw=kwlab(v,n);if(!kw)continue;var it=[];for(i=0;i<rs.length;i++){var r=rs[i],t=String(ptxt(r,n)||'').trim();if(!t)continue;var ls=names(r['answer_kw'+n]||''),who=ls.length?ls[0]:'';var mine=!!(who&&v.me&&N(who)===N(v.me));it.push({m:String(r.model||'').toUpperCase(),t:t,who:who,mine:mine});any=true;}if(it.length)g.push({kw:kw,it:it});}var seen={},k;for(i=0;i<rs.length;i++){k=String(rs[i].model||'').toLowerCase();if(!k)continue;if(!(k in seen))seen[k]=0;for(n=1;n<=3;n++){if(String(ptxt(rs[i],n)||'').trim())seen[k]++;}}var quiet=[];for(k in seen){if(!seen[k])quiet.push(k.toUpperCase());}var h=css6()+'<div class="q6"><div class="qh">What AI actually says</div><div class="qc">';if(!any){h+='<p class="qe">No model described a business today. When they do, their exact words appear here.</p>';}else{for(i=0;i<g.length;i++){h+='<div class="qg"><div class="qq">Asked: '+E(g[i].kw)+'</div>';for(j=0;j<g[i].it.length;j++){var x=g[i].it[j];h+='<div class="qr"><div class="qm2">'+E(x.m)+'</div><p class="qt">'+hlq(x.t,x.who)+((x.who&&!x.mine)?'<span class="qtag">ranked above you</span>':'')+'</p></div>';}h+='</div>';}}var f='Quoted word for word from what each model replied. Each quote describes the business that model ranked first.';if(quiet.length)f+=' No description returned today from: '+E(quiet.join(', '))+'.';h+='<p class="qfoot">'+f+'</p></div></div>';return h;}function DD(rows){var m={},order=[],o=[],i,r,k,cur;function filled(x){var c=0,n,f,a;for(n=1;n<=3;n++){f=x['kw'+n+'_mentioned'];if(f!==undefined&&f!==null&&f!==''){f=String(f).toLowerCase();if(f==='yes'||f==='no')c++;continue;}a=String(x['answer_kw'+n]||'').trim();if(a)c++;}return c;}for(i=0;i<rows.length;i++){r=rows[i];k=String(r.check_date)+'|'+String(r.model||'').toLowerCase();cur=m[k];if(!cur){m[k]=r;order.push(k);continue;}if(filled(r)>=filled(cur))m[k]=r;}for(i=0;i<order.length;i++)o.push(m[order[i]]);return o;}function skin(){return '<style>#aivis2 .thr{display:none}#aivis2 .dial{width:80px;height:80px;flex:0 0 80px}#aivis2 .dial svg circle{stroke-width:9}#aivis2 .dial .dc b{font-size:19px;letter-spacing:-.02em;line-height:1.05}#aivis2 .dial .dc s{font-size:8px;letter-spacing:.08em}#aivis2 .lf{gap:16px;align-items:center}#aivis2 .lf>div:not(.dial){flex:1 1 0;min-width:0;align-self:center}</style>';}function rstat(v,r){var n,f,o={ans:0,named:0,miss:0,tot:0};for(n=1;n<=3;n++){if(!kwlab(v,n))continue;o.tot++;f=r['kw'+n+'_mentioned'];f=(f===undefined||f===null)?'':String(f).toLowerCase();if(f==='yes'){o.ans++;o.named++;}else if(f==='no'){o.ans++;}else{o.miss++;}}return o;}function rcss(){return '<style>#aivis2 .t6{--t6-o:#FF3D14;--t6-n:#0F0638;--t6-l:#EEE9FF;margin:32px 0 0;width:100%}#aivis2 .t6h{font-family:Arial,Helvetica,sans-serif;font-weight:700;letter-spacing:-.06em;font-size:16px;color:var(--tx);margin:0 0 4px}#aivis2 .t6s{font-size:12px;line-height:1.5;color:var(--mu);margin:0 0 16px;max-width:52ch}#aivis2 .t6g{display:grid;grid-template-columns:repeat(4,1fr);gap:16px}#aivis2 .t6t{display:flex;flex-direction:column;align-items:center;gap:4px;opacity:0;animation:t6up 400ms cubic-bezier(.16,1,.3,1) forwards;animation-delay:calc(var(--i) * 80ms)}@keyframes t6up{from{opacity:0;transform:translateY(8px)}to{opacity:1;transform:none}}#aivis2 .t6r{position:relative;width:64px;height:64px}#aivis2 .t6r svg{transform:rotate(-90deg)}#aivis2 .t6k{fill:none;stroke:var(--t6-l);stroke-width:6}#aivis2 .t6a{fill:none;stroke-width:6;stroke-linecap:round;transition:stroke-dashoffset 400ms cubic-bezier(.16,1,.3,1)}#aivis2 .t6a.full{stroke:var(--t6-n)}#aivis2 .t6a.part{stroke:var(--t6-o)}#aivis2 .t6a.none{stroke:transparent}#aivis2 .t6v{position:absolute;inset:0;display:grid;place-items:center;font-family:Arial,Helvetica,sans-serif;font-weight:700;font-size:15px;letter-spacing:-.04em;color:var(--tx)}#aivis2 .t6n{font-size:11px;font-weight:700;letter-spacing:.15em;text-transform:uppercase;color:var(--tx);margin:8px 0 0}#aivis2 .t6c{font-size:11px;line-height:1.4;color:var(--mu);margin:0;text-align:center}#aivis2 .t6c.warn{color:var(--t6-o)}#aivis2.dk .t6{--t6-l:rgba(255,255,255,.14)}#aivis2.dk .t6a.full{stroke:#FFFFFF}@media(max-width:560px){#aivis2 .t6g{grid-template-columns:repeat(2,1fr);gap:16px}}@media(prefers-reduced-motion:reduce){#aivis2 .t6t{animation-duration:.01ms}}</style>';}function rings(v){var rs=trows(v),i,h;if(!rs.length)return '';h=rcss()+'<section class="t6" aria-label="Each AI model today">'+'<h3 class="t6h">Each AI model today</h3>'+'<p class="t6s">How many of your keywords each model names you for. A keyword a model did not return is left out of its total, not counted against you.</p>'+'<div class="t6g">';for(i=0;i<rs.length;i++){var r=rs[i],s=rstat(v,r),nm=String(r.model||'').toUpperCase();var C=163.36,pctv=s.ans?(s.named/s.ans):0,off=C-(C*pctv);var cls=!s.ans?'none':(s.named===s.ans?'full':'part');var val=s.ans?(s.named+'/'+s.ans):'\u2014';var cap=!s.ans?('no answer returned'):(s.miss?(s.miss+' of '+s.tot+' keywords not returned'):('all '+s.tot+' measured'));var lab=nm+(s.ans?(' named you in '+s.named+' of '+s.ans+' answered keywords'):' returned no answer today');h+='<div class="t6t" style="--i:'+i+'"><div class="t6r" role="img" aria-label="'+E(lab)+'">'+'<svg viewBox="0 0 64 64" width="64" height="64" aria-hidden="true">'+'<circle class="t6k" cx="32" cy="32" r="26"></circle>'+'<circle class="t6a '+cls+'" cx="32" cy="32" r="26" style="stroke-dasharray:'+C+';stroke-dashoffset:'+off.toFixed(2)+'"></circle>'+'</svg><b class="t6v">'+val+'</b></div>'+'<p class="t6n">'+E(nm)+'</p><p class="t6c'+((!s.ans||s.miss)?' warn':'')+'">'+E(cap)+'</p></div>';}h+='</div></section>';return h;}function theme(el){
