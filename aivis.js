...
+'<span><span class="pill"><i></i>'+E(v.pill)+'</span><s class="thr">'+E(v.pillNote)+'</s></span></div>'
+'<div class="bd">'
...
function mount(){
 ...
 document.addEventListener('click',function(e){
  if(e.target&&e.target.closest&&e.target.closest('.theme-toggle')){
   try{var at=document.querySelector('button.nav-tab.active')||[].slice.call(document.querySelectorAll('button.nav-tab')).filter(function(b){return /AI Visibility/i.test(b.textContent||'');})[0];
   if(at){var lbl=(at.textContent||'').trim();setTimeout(function(){var n=[].slice.call(document.querySelectorAll('button.nav-tab')).filter(function(b){return (b.textContent||'').trim()===lbl;})[0];if(n&&!/active/.test(n.className))n.click();},250);}}catch(er){}
