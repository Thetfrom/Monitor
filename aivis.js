...
function mount(){
 ...
 document.addEventListener('click',function(e){
  if(e.target&&e.target.closest&&e.target.closest('.theme-toggle')){
   try{var tabs=[].slice.call(document.querySelectorAll('button.nav-tab'));var at=tabs.filter(function(b){return /active|current|selected/i.test(b.className)||b.getAttribute('aria-selected')==='true';})[0];if(!at&&document.getElementById('aivis2')){at=tabs.filter(function(b){return /AI Visibility/i.test(b.textContent||'');})[0];}if(at){var lbl=(at.textContent||'').trim();var t0=Date.now();var iv=setInterval(function(){var nt=[].slice.call(document.querySelectorAll('button.nav-tab')).filter(function(b){return (b.textContent||'').trim()===lbl;})[0];var on=nt&&(/active|current|selected/i.test(nt.className)||nt.getAttribute('aria-selected')==='true');if(nt&&!on){nt.click();clearInterval(iv);}else if(on||Date.now()-t0>5000){clearInterval(iv);}},120);}}catch(er){}
