function aiUntracked(todayRows,esc){return null;/*disabled: cannot reach masterRecord from this scope*/try{if(!todayRows||!todayRows.length)return null;var mr=(window.state&&window.state.data&&window.state.data.masterRecord)||{};var E=esc||function(x){return String(x==null?"":x);};var norm=function(x){return String(x==null?"":x).toLowerCase().replace(/\s+/g," ").trim();};var me=norm(mr.business_name);var rivals=[norm(mr.competitor_1_name),norm(mr.competitor_2_name),norm(mr.competitor_3_name)].filter(Boolean);var counts={},labels={},total=0,i,n,raw,parts,j,nm,key,seen;for(i=0;i<todayRows.length;i++){for(n=1;n<=3;n++){raw=todayRows[i]["answer_kw"+n];if(!raw||!String(raw).trim())continue;total++;seen={};parts=String(raw).split(";");for(j=0;j<parts.length;j++){nm=String(parts[j]).trim();if(!nm||nm.length>60)continue;key=norm(nm);if(!key||seen[key])continue;seen[key]=1;counts[key]=(counts[key]||0)+1;if(!labels[key])labels[key]=nm;}}}if(!total)return null;var rivalTop=0,k;for(k in counts){if(rivals.indexOf(k)>=0&&counts[k]>rivalTop)rivalTop=counts[k];}var best=null,bestN=0;for(k in counts){if(!k||k===me)continue;if(me&&(k.indexOf(me)>=0||me.indexOf(k)>=0))continue;if(rivals.indexOf(k)>=0)continue;if(counts[k]>bestN){bestN=counts[k];best=k;}}if(!best||bestN<2||bestN<rivalTop)return null;return "Worth knowing: "+E(labels[best])+" is named in "+bestN+" of "+total+" stored answers today, more than any rival you track, and is not on your competitor list.";}catch(e){return null;}}function aiBrief(todayRows,allRows,prevD,esc){var a=null,b=null;try{a=aiGapLine(todayRows,allRows,prevD,esc);}catch(e){}try{b=aiUntracked(todayRows,esc);}catch(e){}if(a&&b)return a+" "+b;return a||b||null;}
function aiGapLine(todayRows,allRows,prevD,esc){return null;/*disabled: cannot reach masterRecord from this scope*/try{if(!todayRows||!todayRows.length)return null;var mr=(window.state&&window.state.data&&window.state.data.masterRecord)||{};var biz=String(mr.business_name||"").toLowerCase().trim();var kws=[mr.target_keyword_1,mr.target_keyword_2,mr.target_keyword_3];var E=esc||function(x){return String(x==null?"":x);};var gaps=[],i,n,r,v,kw;for(i=0;i<todayRows.length;i++){r=todayRows[i];for(n=1;n<=3;n++){v=r["kw"+n+"_mentioned"];if(String(v)==="no"){kw=kws[n-1];if(!kw)continue;var isBrand=biz&&String(kw).toLowerCase().indexOf(biz)>=0;gaps.push({model:String(r.model||""),kw:String(kw),n:n,brand:!!isBrand});}}}if(!gaps.length)return null;gaps.sort(function(a,b){return (b.brand?1:0)-(a.brand?1:0);});var g=gaps[0];var tot=0,miss=0;if(allRows&&allRows.length){for(i=0;i<allRows.length;i++){r=allRows[i];if(String(r.model||"")!==g.model)continue;v=r["kw"+g.n+"_mentioned"];if(String(v)==="yes"||String(v)==="no"){tot++;if(String(v)==="no")miss++;}}}var name=g.model.toUpperCase();if(g.brand){return "Standing gap: "+E(name)+" does not name you for your own business name keyword \u201c"+E(g.kw)+"\u201d."+(tot?" That has been true in "+miss+" of the last "+tot+" stored checks for this model.":"");}return "Gap today: "+E(name)+" does not name you for \u201c"+E(g.kw)+"\u201d."+(tot?" True in "+miss+" of the last "+tot+" stored checks for this model.":"");}catch(e){return null;}}
  ...
  function renderAI() {
    ...
    if (!checks.length) {
      ...
      var actA = chg.length
        ...
        : (prevD ? (aiBrief((typeof todayRows!=='undefined'?todayRows:null),(typeof rowsAll!=='undefined'?rowsAll:null),prevD,(typeof escAI!=='undefined'?escAI:null)) || ('Nothing changed since ' + escAI(prevD) + '. No action needed today.')) : 'This is your first check, so there is nothing to compare yet. We start tracking changes from tomorrow.');
